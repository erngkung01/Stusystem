...
$query_Student = sprintf("SELECT a.*,b.*,c.* FROM tbl_student as a , tbl_prefix as b , tbl_address as c WHERE (a.studentID = %s or a.student_name like %s or a.student_surname like %s ) and( a.PrefixCode=b.PrefixCode and a.studentID=c.studentID)", GetSQLValueString($colname_Student, "text"),GetSQLValueString($colname2_Student . "%", "text"),GetSQLValueString($colname3_Student . "%", "text"));
$query_limit_Student = sprintf("%s LIMIT %d, %d", $query_Student, $startRow_Student, $maxRows_Student);
$Student = mysqli_query($stusystem, $query_limit_Student) or die(mysqli_error($stusystem));
$row_Student = mysqli_fetch_assoc($Student);

if (isset($_GET['totalRows_Student'])) {
  $totalRows_Student = $_GET['totalRows_Student'];
} else {
  $all_Student = mysqli_query($stusystem, $query_Student);
  $totalRows_Student = mysqli_num_rows($all_Student);
}
$totalPages_Student = ceil($totalRows_Student/$maxRows_Student)-1;
...
?>
    ...
    <div class="col-sm-8">
    <input name="word" type="text" class="form-control" id="word" placeholder="ใส่ชื่อ / นามสกุลหรือรหัสนักเรียน">
    </div>
