...
include("../../student/studentaward/dw-upload.inc.php");?>
<?php
if (!isset($_SESSION)) {
  session_start();
}
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "update")) {
  ...
  $updateSQL = sprintf("UPDATE tbl_teacheraward SET AwardYear=%s, AwardSemester=%s, AwardName=%s, AwardDetail=%s, AwardPic=%s, AwardDate=%s, AwardBy=%s, TeacherID=%s, AwardTypeID=%s WHERE AwardID=%s",
		 ...
		 $updateSQL = sprintf("UPDATE tbl_teacheraward SET AwardYear=%s, AwardSemester=%s, AwardName=%s, AwardDetail=%s, AwardDate=%s, AwardBy=%s, TeacherID=%s, AwardTypeID=%s WHERE AwardID=%s",
                       GetSQLValueString($_POST['AwardYear'], "int"),
                       GetSQLValueString($_POST['AwardSemester'], "int"),
                       GetSQLValueString($_POST['AwardName'], "text"),
                       GetSQLValueString($_POST['AwardDetail'], "text"),
...
if (isset($_GET['AwardID'])) {
  $colname_teacheraward = $_GET['AwardID'];
}
//42320819($database_stusystem, $stusystem);
$query_teacheraward = sprintf("SELECT * FROM tbl_teacheraward WHERE AwardID = %s", GetSQLValueString($colname_teacheraward, "int"));
...
<head>
<meta charset="utf-8">
<title>เพิ่มข้อมูลผลงาน/รางวัลอาจารย์</title>
...
<div class="container">
<a href="index.php?TeacherID=<?php echo $row_teacher['TeacherID']; ?>" class="btn btn-primary">กลับไปหน้าข้อมูลผลงาน/รางวัลอาจารย์</a>
 <h2 style="text-align:center;">ปรับปรุงข้อมูลผลงาน/รางวัลอาจารย์</h2>
  ...
  <table class="table table-condensed">
    ...
    <tbody>
      <tr>
       ...
       <tr>
        <td>ชื่อ</td>
        <td><input name="student_name" type="text" required="required" class="form-control" id="StudentID" placeholder="ใส่ชื่อ" value="<?php echo $row_teacher['name']; ?>" readonly></td>
      </tr>
       <tr>
        <td>นามสกุล</td>
        <td><input name="student_surname" type="text" required="required" class="form-control" id="StudentID" placeholder="ใส่นามสกุล" value="<?php echo $row_teacher['surname']; ?>"  readonly></td>
      </tr>
      <tr>
      <td>ชื่อผลงานรางวัล</td>
      <td><input name="AwardName" type="text" class="form-control" id="AwardName" value="<?php echo $row_teacheraward['AwardName']; ?>"></td>
      </tr>
      <tr>
      <td>ปีการศึกษา</td>
...
do {  
?>
        <option value="<?php echo $row_academicYear['AcademicYearsID']?>"<?php if (!(strcmp($row_academicYear['AcademicYearsID'], $row_teacheraward['AwardYear']))) {echo "selected=\"selected\"";} ?>><?php echo $row_academicYear['AcademicYears']?></option>
        <?php
...
do {  
?>
        <option value="<?php echo $row_Semester['SemesterID']?>"<?php if (!(strcmp($row_Semester['SemesterID'], $row_teacheraward['AwardSemester']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Semester['Semester']?></option>
        <?php
...
?>
      ...
      <td>
      	<script src="../../student/studentaward/ckeditor.js"></script>

		<!-- Sample 2 -->
		<textarea cols="80" id="AwardDetail" name="AwardDetail" class="ckedtor" rows="10">
		<?php echo $row_teacheraward['AwardDetail']; ?>
      ...
      <td>
        ...
        <img src="images/<?php echo $row_teacheraward['AwardPic']; ?>" id="" alt="" width="150" height="150"></td>
      </tr>
       <tr>
      <td>วันที่ได้รับ </td>
       <td> <input type="text" name="AwardDate" id="AwardDate" value="<?php
		if($row_teacheraward['AwardDate']!=""){
		$date = $row_teacheraward['AwardDate']; 
		$show=explode("-",$date);
		$date1 = $show[0]+543;
		$date2 = $show[1];
		$date3 = $show[2];
...
$(function(){
    ...
    $("#AwardDate").on("mouseenter mouseleave",function(e){
      ...
      <td><input name="AwardBy" type="text" class="form-control" id="AwardBy" value="<?php echo $row_teacheraward['AwardBy']; ?>"></td>
      </tr>
       <tr>
...
do {  
?>
        <option value="<?php echo $row_Awardtype['AwardTypeID']?>"<?php if (!(strcmp($row_Awardtype['AwardTypeID'], $row_teacheraward['AwardTypeID']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Awardtype['AwardTypeName']?></option>
        <?php
...
?>
      ...
      <td colspan="2" style="text-align:center;"><input name="inserstudentaward" type="submit" id="inserstudentaward" value="ปรับปรุงข้อมูลผลงาน/รางวัลนักเรียน" class="btn btn-success" ><input name="AwardID" type="hidden" id="AwardID" value="<?php echo $row_teacheraward['AwardID']; ?>"></td>
        </tr>
    </tbody>
  </table>
</div>
