...
$colname_teacher = "-1";
if (isset($_GET['word'])) {
  $colname_teacher = $_GET['word'];
}
$colname2_teacher = "-1";
if (isset($_GET['word'])) {
  $colname2_teacher = $_GET['word'];
}
$colname3_teacher = "''";
if (isset($_GET['word'])) {
  $colname3_teacher = $_GET['word'];
}
$colname4_teacher = "''";
if (isset($_GET['word'])) {
  $colname4_teacher = $_GET['word'];
}
...
if(!empty($_GET['PositionID']) && $_GET['PositionID']>0){ //คำนำหน้าชื่อ
	$arrWhere[]= 'a.PositionID='.$_GET['PositionID'];
...
//42320819($database_stusystem, $stusystem);
$query_teacher = sprintf("SELECT a.*,b.*,c.* FROM tbl_teacher as a,tbl_prefix as b, tbl_teacheraddress as c WHERE (a.TeacherID=%s or a.PersonID=%s or a.name like %s or a.surname like %s  )  and ( a.PrefixCode=b.PrefixCode and a.TeacherID=c.TeacherID) ".$strSQLWhere." ORDER BY a.TeacherID DESC", GetSQLValueString($colname_teacher, "text"),GetSQLValueString($colname2_teacher, "text"),GetSQLValueString($colname3_teacher . "%", "text"),GetSQLValueString($colname4_teacher . "%", "text"));
...
?>
    ...
    <div class="col-sm-8">
    <input name="word" type="text" class="form-control" id="word" placeholder="ใส่ชื่อ / นามสกุล / รหัสอาจารย์ หรือ รหัสประชาชน" value="<?php echo $_GET['word']; ?>">
    </div>
...
?>
 ...
 <div class="col-sm-12"><br></div>
  ...
  <a href="teacherqr/index.php" class="btn btn-success">ค้นหาโดย QRCode</a>
   ...
   จำนวน  <strong style="color:#000DFF;"><?php echo $totalRows_teacher ?></strong> คน 
</div>
