...
<?php include ("../../teacher-menu.php");?>
  ...
  <div class="col-sm-7">
   ...
   <a href="insert.php?TeacherID=<?php echo $row_teacher['TeacherID']; ?>" class="btn btn-success">เพิ่มปีการศึกษา</a>
    
    </div>
    <?php if ($totalRows_dutyyear > 0) { // Show if recordset not empty ?>
  <h3 style="text-align:left;">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <?php do { ?>
        <tr>
          <td><?php echo $row_dutyyear['AcademicYears']; ?></td>
            
          <td><a href="../teacherduty/index.php?TeacherID=<?php echo $row_teacher['TeacherID']; ?>&dutyyearID=<?php echo $row_dutyyear['dutyyearID']; ?>" class="btn btn-primary">จัดการข้อมูล</a></td>
          <td><a href="delete.php?dutyyearID=<?php echo $row_dutyyear['dutyyearID']; ?>&TeacherID=<?php echo $row_teacher['TeacherID']; ?>" class="btn btn-danger"onclick="return confirm('คุณแน่ใจที่จะลบข้อมูลนี้')">ลบ</a></td>
            
          </tr>
          <?php } while ($row_dutyyear = mysqli_fetch_assoc($dutyyear)); ?>
      </tbody>
    </table>
      <?php } // Show if recordset not empty ?>
      <?php if ($totalRows_dutyyear == 0) { // Show if recordset empty ?>
       <br>
        <table class="table table-hover">
    <tbody>
      <tr>
